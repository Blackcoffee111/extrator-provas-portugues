...
def merge_review_meta(workspace_dir):
    ...
    if not review_path.exists():
        print(f"ERRO: {review_path} não encontrado.")
        return
    if not meta_path.exists():
        print(f"ERRO: {meta_path} não encontrado.")
        return

    print(f"Lendo {review_path}...")
    review_list = json.loads(review_path.read_text(encoding="utf-8"))
    print(f"Lendo {meta_path}...")
    ...
    for r in review_list:
        ...
        if id_ in meta_by_id:
            ...
            max_ordem += 1
            m = meta_fallback(id_, max_ordem)
            print(f"Aviso: Item {id_} não encontrado no meta, usando fallback.")
        
        # Merge review override onto meta base
        merged_item = {**m, **r}
        merged.append(merged_item)

    print(f"Gravando {raw_path} com {len(merged)} itens...")
    raw_path.write_text(json.dumps(merged, indent=2, ensure_ascii=False), encoding="utf-8")
    print("Sincronização concluída.")

if __name__ == "__main__":
    ws = Path("/Users/adrianoushinohama/Desktop/Exames Nacionais/Provas de portugues/workspace/EX-Port639-F2-2024_net")
    merge_review_meta(ws)
